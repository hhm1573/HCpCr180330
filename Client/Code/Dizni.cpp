...
CDizni::CDizni(LPDIRECT3DDEVICE9 pDevice)
	:CModel(pDevice)
...
CDizni::CDizni(const CDizni & Ref)
	: CModel(Ref)
...
HRESULT CDizni::Initialize()
{
	...
	m_pComponent_Transform->Set_Information(CTransform::INFO_POSITION, &My_Vec3(424.f, 6.5f, 286.5f));
...
HRESULT CDizni::Initialize_Prototype()
{
	return CModel::Initialize_Prototype(L"../Bin/Data/Character/Dizny_Cart_Client.dat");
}

My_Int CDizni::Update(const My_Float & fTimeDelta)
{
	if (mIsStop)
		return My_Int();
	...
	if (mIsTargeted)
	{
		...
		dynamic_cast<CRay_Tri2*>(pRayTri2)->Set_Target_Num(2);
		...
		dynamic_cast<CRay_Tri*>(pRayTri)->Set_Target_Num(2);
...
My_Int CDizni::LastUpdate(const My_Float & fTimeDelta)
{
	if (!mIsGoal)
	{
...
void CDizni::Load(int recordNum)
{
	HANDLE hFile = CreateFile(L"../Bin/Data/Record7.dat", GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
void CDizni::Collision(const My_Float& fTimeDelta)
{
	if (2 == mState) // 아이템 박스와 충돌
	{
		if (mItemVec.size() < 2)
		{
			...
			{
				if (CRankingManager::BAZZI == CRankingManager::GetInstance()->GetFrontRacer(CRankingManager::DIZNI))
				{
					randItem = rand() % 2;
	...
	if (3 == mState && (mItemState == BANANA || mItemState == NONE))	// 바나나와 충돌했을 때
	{
		if (0.f == mAccSpeed)
		{
			mItemState = BANANA;
	...
	if (4 == mState && (mItemState == WATERBOMB || mItemState == NONE))		// 물폭탄과 충돌했을 때
	{
		if (true == mIsFirst)
		{
	...
	if (6 == mState && mIsMissileTarget && (mItemState == MISSILE || mItemState == NONE)) // 미사일과 충돌
	{
		if (true == mIsFirst)
		{
			mItemState = MISSILE;
	...
	if (9 == mState && mIsWaterFlyTarget && (mItemState == WATERFLY || mItemState == NONE))		// 물파리와 충돌했을 때
	{
		if (true == mIsFirst)
		{
			mItemState = WATERFLY;
	...
	mCollisioner->Add_Collision(*m_pComponent_Transform->Get_Information(CTransform::INFO_POSITION), 1.0f, 8, &mState);
	mCollisioner->Add_CollisionAABB(vPos - My_Vec3(0.25f, 0.25f, 0.25f), vPos + My_Vec3(0.25f, 0.25f, 0.25f), 3, &mReceive, &m_vCol);
}

void CDizni::UseItem(const My_Float& fTimeDelta)
{
	if (mItemVec.size() > 0)
		mItemUseTime += fTimeDelta;

	if (0 == mState && mItemVec.size() > 0 && mItemUseTime > 5.f)
	{
		switch (mItemVec[0])		
		{
			case 0:		// 구름
...
void CDizni::ItemMissile()
{
	My_Int targetNum;
	CRankingManager::Racer frontRacer = CRankingManager::GetInstance()->GetFrontRacer(CRankingManager::DIZNI);

	if (CRankingManager::BAZZI == frontRacer)
		targetNum = 0;
	else if (CRankingManager::DAO == frontRacer)
		targetNum = 1;
	else
		return;
...
void CDizni::ItemWaterFly()
{
	CRankingManager::Racer frontRacer = CRankingManager::GetInstance()->GetFrontRacer(CRankingManager::DIZNI);

	if (CRankingManager::DIZNI != frontRacer && CRankingManager::RACER_END != frontRacer)
	{
...
HRESULT CDizni::Update_Collision_Tetris(const My_Float & fTimeDelta)
{
	if (m_fFlatTime != 0.0f) {
...
CDizni * CDizni::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CDizni*		pInstance = new CDizni(pDevice);
...
CGameObject * CDizni::Clone(void)
{
	CDizni*		pInstance = new CDizni(*this);
...
My_uLong CDizni::ReleaseMemory(void)
{
