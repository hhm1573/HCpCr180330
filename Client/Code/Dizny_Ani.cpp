#include "stdafx.h"
#include "..\Header\Dizny_Ani.h"
#include "GameObjectMgr.h"



CDizny_Ani::CDizny_Ani(LPDIRECT3DDEVICE9 pDevice)
	: CGameObject(pDevice)
...
CDizny_Ani::CDizny_Ani(const CDizny_Ani & Ref)
	: CGameObject(Ref)
...
void CDizny_Ani::SetRanking(My_Int rank)
{
	if (0 == rank)
...
HRESULT CDizny_Ani::Initialize_Component()
{
	...
	// For.Com_Texture
	m_pTextureCom = (CTexture*)m_pComponentMgr->Clone_Component(SCENE_STAGE, L"Component_Texture_Dizny_Ani");
	if (nullptr == m_pTextureCom)
		return E_FAIL;
	if (FAILED(CGameObject::Add_Component(L"Component_Texture_Dizny_Ani", m_pTextureCom)))
		return E_FAIL;

	return NOERROR;
}

HRESULT CDizny_Ani::Initialize_Prototype()
{
	return NOERROR;
}

My_Int CDizny_Ani::Update(const My_Float & fTimeDelta)
{
...
My_Int CDizny_Ani::LastUpdate(const My_Float & fTimeDelta)
{
...
void CDizny_Ani::Set_RenderState()
{
...
void CDizny_Ani::Release_RenderState()
{
...
CDizny_Ani * CDizny_Ani::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CDizny_Ani*	pInstance = new CDizny_Ani(pDevice);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("CDizny_Ani Prototype Created Failed");
...
CGameObject * CDizny_Ani::Clone()
{
	CDizny_Ani*	pInstance = new CDizny_Ani(*this);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("CDizny_Ani Created Failed");
		Safe_Release(pInstance);
	}
...
My_uLong CDizny_Ani::ReleaseMemory()
{
