#include "stdafx.h"
#include "..\Header\Logo_BG.h"


CLogo_BG::CLogo_BG(LPDIRECT3DDEVICE9 pDevice)
	: CGameObject(pDevice)
...
CLogo_BG::CLogo_BG(const CLogo_BG & Ref)
	: CGameObject(Ref)
...
HRESULT CLogo_BG::Initialize_Component()
{
	...
	// For.Com_Texture
	m_pTextureCom = (CTexture*)m_pComponentMgr->Clone_Component(SCENE_STAGE, L"Component_Texture_LogoBack");
	if (nullptr == m_pTextureCom)
		return E_FAIL;
	if (FAILED(CGameObject::Add_Component(L"Component_Texture_LogoBack", m_pTextureCom)))
		return E_FAIL;
...
My_Int CLogo_BG::Update(const My_Float & fTimeDelta)
{
...
My_Int CLogo_BG::LastUpdate(const My_Float & fTimeDelta)
{
	return My_Int();
}

void CLogo_BG::Render()
{
	if (nullptr == m_pBufferCom)
		return;
...
void CLogo_BG::Release_RenderState()
{
...
CLogo_BG * CLogo_BG::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CLogo_BG*	pInstance = new CLogo_BG(pDevice);
...
CGameObject * CLogo_BG::Clone()
{
	CLogo_BG*	pInstance = new CLogo_BG(*this);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX("CLogo_BG Created Failed");
		Safe_Release(pInstance);
	}
...
My_uLong CLogo_BG::ReleaseMemory()
{
